...
AC_PATH_X_DIRECT
AC_PATH_X_XMKMF
if test "$ac_x_includes" = NO || test -z "$ac_x_includes"; then
  AC_MSG_ERROR([Can't find X includes. Please check your installation and add the correct paths!])
fi

if test "$ac_x_libraries" = NO || test -z "$ac_x_libraries"; then
...
AC_DEFUN(AC_HAVE_GL,
 ...
 if test "x$gl_test" = xno; then
   ...
   [
    ...
    ac_save_cflags="$CFLAGS"
    ac_glflags="-lGL -lGLU"
    LDFLAGS="$LDFLAGS $GL_LDFLAGS $all_libraries $ac_glflags -L/usr/X11R6/lib -lX11 -lXext -lm $LIBSOCKET"
    CFLAGS="$CFLAGS $X_INCLUDES"
  ...
  if test "$ac_cv_have_gl" = no; then
    ...
    if test "$GL_LDFLAGS" = ""; then
       GLLIB="$ac_glflags "'$(LIB_X11)'
    else
       GLLIB="$GL_LDFLAGS $ac_glflags "'$(LIB_X11)'
    fi
...
if AC_TRY_EVAL(ac_try) && test -s conftest.out; then 
    ac_cv_maxpathlen=`sed 's#KDE_HELLO \([0-9]*\).*#\1#' conftest.out`
else
